---
title: Quantum Fourier Transform
date: 2020-06-19 05:04
draft: true
---

This post is the first iteration of an $N$-part series of blog posts aimed at
attempting to understand some of the ideas behind Shor's quantum algorithm for factoring
composite numbers.

I would recommend familiarity with some basic linear algebra and bra-ket notation.
The following lecture notes on [bra-ket](https://ocw.mit.edu/courses/physics/8-05-quantum-physics-ii-fall-2013/lecture-notes/MIT8_05F13_Chap_04.pdf)
notation are sufficient.

### Contents

1. [Not So Quantum Fourier Transform](#classical)
   1. [Definitions](#classical_definitions)
   2. [Examples](#classical_examples)
   3. [Properties of interest](#classical_properties)
2. [Going Quantum](#quantum)
   1. [Definitions](#quantum_definitions)
   2. [Examples](#quantum_examples)
   3. [Complexity](#quantum_complexity)
3. [Middle Ground](#semi)
   1. [We shall see](#provosional)
4. [Notes](#notes)
5. [Conclusion](#conclusion)

### Not So Quantum Fourier Transform {#classical}
dsakdjskldjklaj
dsadlklk

#### Definitions {#classical_definitions}
djaskjdlkasjd

#### Examples {#classical_examples}
dsakldjlkasjd

#### Properties of interest {#classical_properties}
dsajkldjakls

#### Complexity {#classical_complexity}
dsjakdjklasj

### Going Quantum {#quantum}
djsakdjkl

#### Definitions {#quantum_definitions}
dsalkdjk

#### Examples {#quantum_examples}
dskadjlkj

#### Complexity {#quantum_complexity}
dsjakldjlkasjkdj

### Middle Ground {#semi}
dsjakldjlkasjdkj

### Notes
dsjkadjaklsdj

### Conclusion
dsakdjkklasjdkl
